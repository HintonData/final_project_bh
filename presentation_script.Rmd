...
So let's move onto the analysis, and began answering the business questions.
...
So for this, I decided to look at the outcomes for different animals across time.
```{r, echo=FALSE, message=FALSE, error=FALSE, eval=TRUE}
animal_outcomes %>% 
  drop_na() %>% 
  ...
  ungroup() %>% 
  filter(outcome == "Rehomed" | outcome == "Euthanized") %>%   filter(animal_type != "Horses" & animal_type != "Livestock") %>%
  ggplot(aes(x = year, y = total_injuries, color = outcome)) +
  geom_line() +
  facet_wrap(~animal_type, scales = "free_y") +
  labs(x = "Date", y = "Number of Cases", color = "Outcome", title = "Outcomes Across Time for Animals") +
  scale_fill_discrete(name = "Complaint Type") +
  scale_y_continuous(labels = scales::comma) +
  theme_economist() +
  theme(axis.text.x = element_text(vjust = 1, family = "mono", face = "bold", size = 10),
        ...
        plot.title = element_text(hjust = 0.4),
        legend.text = element_text(size = 8, family = "mono", face = "bold"),
        strip.text = element_text(size = 9, family = "mono", face = "bold"))
```

As you can see here, there's some noticeable trends. The euthanizations have gone way up for wildlife! Looking into this further, I discovered Queensland was responsible for this trend. They have steadily been increasing euthanizations of wildlife overtime, while for other regions this hasn't been the case at all. Why this is the case is something I recommend the RSCPA look into more.

Another thing to note is the steady decline in euthanizations for everything else. For Other Animals as well as for cats, there is a trend of euthanizations going down and rehoming going up. This is good news and means that the RSPCA is accomplishing it's goal.

For Dogs, the trend is not so clear, it appears rehoming is on the decline; however euthanizations have fallen dramatically and this is definitely a good sign.



## Are there any major differences between the way different States of Australia deal with animals?

Next, I broke things down by region and looked at the rates of reclaim. For the sake of simplicity, we'll look at three of Australia's regions.
```{r, echo=FALSE, message=FALSE, error=FALSE, eval=TRUE}
animal_outcomes %>% 
  drop_na() %>% 
  ...
  filter(region == "QLD" | region == "VIC" | region == "TAS") %>% 
  ggplot(aes(x = year, y = total_injuries, color = outcome)) +
  geom_line(colour = "blue") +
  facet_wrap(~region, scales = "free_y") +
  labs(x = "Date", y = "Number of Cases", color = "Outcome", title = "Animals Reclaimed by Territory") +
  scale_fill_discrete(name = "Complaint Type") +
  scale_y_continuous(labels = scales::comma) +
  theme_economist() +
  theme(axis.text.x = element_text(vjust = 1, family = "mono", face = "bold", size = 10),
        ...
        axis.text.y = element_text(size = 9, family = "mono"),
        axis.title.x = element_text(vjust = -1),
        title = element_text(family = "mono", face = "bold", vjust = 2.4),
        plot.title = element_text(hjust = 0.4),
        legend.text = element_text(size = 8, family = "mono", face = "bold"),
        strip.text = element_text(size = 12, family = "mono", face = "bold"))
```
So some interesting things here. Queensland has been steadily climbing, and this speaks well to the RSPCA's efforts. Victoria, has had quite a bit of variability, but as you can see has been on the decline since about 2009. And Tasmania has had a steep decline since about 2016.
This suggests to me that more initiatives and resources might need to be spent on Victoria.


## Are there any anomalies in the data?

There are a few actually, but there is one I will focus on.
...
animal_outcomes %>% 
  ...
  geom_line() +
  facet_wrap(~animal_type, scales = "free_y") +
  labs(x = "Date", y = "Number of Cases", color = "Region", title = "Wildlife Cases by Territory") +
  scale_fill_discrete(name = "Complaint Type") +
  scale_y_continuous(labels = scales::comma) +
  theme_economist() +
  theme(axis.text.x = element_text(vjust = 1, family = "mono", face = "bold", size = 10),
        ...
        title = element_text(family = "mono", face = "bold"),
        plot.title = element_text(hjust = 0.4),
        legend.text = element_text(size = 8, f amily = "mono", face = "bold"),
        legend.title = element_blank(),
        strip.text = element_blank())
```

As you can see Western Australia had a huge spike in wildlife in 2006. Given that Western Australia normally accounts for a much smaller number of cases, this is quite stark. Upon further investigation, I discovered that in March 2006, cyclone Emma (a tropical storm) swept over Western Australia. Though a weak storm, the rainfall caused mass flooding. It's very possible that this is the reason for the proportionally large spike in wildlife cases. This also lead to a spike in euthanasia, as unfortunately for wild animals this is often a necessary course of action.
...
The RSPCA and local councils have got good, quality rich data that have shown some interesting trends over time broken down by region and areas.

My initial analysis has confirmed some interesting trends, some of which indicate good progress towards the RSPCA's stated goals; for example, euthanizations trending downwards in certain species.

Further investigation with the RSPCA would allow more detailed analysis in the areas that they're interested in, and allow them to make better informed business decisions.
